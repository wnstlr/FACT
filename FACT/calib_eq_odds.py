"""
Adpated from https://github.com/gpleiss/equalized_odds_and_calibration
"""
...
def run_calib_eq_odds(data_filename, cost_constraint='weighted', trial=10):
    ...
    # compute distance
    eo_dist_original = np.abs(group_0_test_model.tpr() - group_1_test_model.tpr()) + np.abs(group_0_test_model.fpr() - group_1_test_model.fpr())
    eo_dist_after = np.abs(calib_eq_odds_group_0_test_model.tpr() - calib_eq_odds_group_1_test_model.tpr()) + np.abs(calib_eq_odds_group_0_test_model.fpr() - calib_eq_odds_group_1_test_model.fpr())
    n1 = group_1_test_model.num_samples()
    n0 = group_0_test_model.num_samples()
    n = n1 + n0
    eo_acc_original = (group_1_test_model.accuracy() * n1 + group_0_test_model.accuracy() * n0) / n
    eo_acc_after = (calib_eq_odds_group_1_test_model.accuracy() * n1 + calib_eq_odds_group_0_test_model.accuracy() * n0) / n
    print(eo_dist_original, eo_acc_original)
    f = open('../notebooks/calib_eqodd_result_trial%d_%s.csv'%(trial, cost_constraint), 'w')
    f.write('%f,%f\n'%(eo_dist_original, eo_acc_original))
    f.write('%f,%f'%(eo_dist_after, eo_acc_after))
    f.close()

    return group_0_test_model, group_1_test_model, calib_eq_odds_group_0_test_model, calib_eq_odds_group_1_test_model

if __name__ == '__main__':
    for i in range(10):
        run_calib_eq_odds('../notebooks/adult_predictions.csv', trial=i, cost_constraint='fnr')
        run_calib_eq_odds('../notebooks/adult_predictions.csv', trial=i, cost_constraint='fpr')
        run_calib_eq_odds('../notebooks/adult_predictions.csv', trial=i, cost_constraint='weighted')
