...
class Model(namedtuple('Model', 'pred label')):
    ...
    def eq_odds(self, othr, mix_rates=None):
        ...
        ## change predictions for group0 model
        ...
        ## change predictions for group1 model
    ...
    def eq_odds_optimal_mix_rates(self, othr):
        ...
        # output: Pr(y\tilde=1|y\hat=1, A=0), Pr(y\tilde=1|y\hat=0, A=0), 
...
def run_eq_odds(data_filename, trial=10):
    dataname = data_filename.split('_')[0]
    ...
    test_data = test_and_val_data
    val_data = test_and_val_data
    ...
    eo_dist_after = np.abs(eq_odds_group_0_test_model.tpr() - eq_odds_group_1_test_model.tpr()) + np.abs(eq_odds_group_0_test_model.fpr() - eq_odds_group_1_test_model.fpr())
    ...
    eo_acc_after = (eq_odds_group_1_test_model.accuracy() * n1 + eq_odds_group_0_test_model.accuracy() * n0) / n
    print(eo_dist_original, eo_acc_original)
    f = open('../notebooks/%s_eqodd_result_trial%d.csv'%(dataname,trial), 'w')
    f.write('%f,%f\n'%(eo_dist_original, eo_acc_original))
    f.write('%f,%f'%(eo_dist_after, eo_acc_after))
    f.close()

    return group_0_test_model, group_1_test_model, eq_odds_group_0_test_model, eq_odds_group_1_test_model
    

if __name__ == '__main__':
    for i in range(10):
        run_eq_odds('../notebooks/unbiased_predictions.csv', trial=i)
"""
Demo
if __name__ == '__main__':
